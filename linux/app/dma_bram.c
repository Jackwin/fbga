...
//        memcpy(DestArray, mapped_dev_base_2, (BUFFER_BYTESIZE ));
	...
	int i;
	int * data_ptr;
	data_ptr = (int *)mapped_dev_base_2;
	for (i = 0; i < 32; i++) {
		printf("data_ptr[%d] is %x.\n", i, data_ptr[i]);
	}
...
/*for (Index = 0; Index < (BUFFER_BYTESIZE/4); Index++)
    ...
    off_t dev_base_3 = PL_BRAM_SRC_ADDRESS;
	memfd_3 = open("/dev/mem", O_RDWR | O_SYNC);
       if (memfd_3 == -1)
       {
           printf("Can't open /dev/mem.\n");
           exit(0);
       ...
       // Map one page of memory into user space such that the device is in that page, but it may not
       // be at the start of the page.
       mapped_base_3 = mmap(NULL, MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, memfd_3, dev_base_3);
	...
	if (munmap(mapped_base_3, MAP_SIZE) == -1)
        {
            printf("Can't unmap memory from user space.\n");
            exit(0);
        }
